...
export async function carregarNotificacoes(hoje: Date = new Date()): Promise<Notificacao[]> {
  ...
  const [passivosAtivos, contas, metasAtivas, configuracao, snapshots, passivosComCiclos, passivosRecentesSemMeta, passivosComCronograma] =
    await Promise.all([
      ...
      prisma.meta.findMany({ where: { status: StatusMeta.ATIVA }, include: { alocacoes: true } }),
